...
namespace MultiShop.WebUI.Services.OrderServices.OrderOrderingServices
{
    public class OrderOrderingService : IOrderOrderingService
    {
        ...
        public async Task CreateOrderAsync(CreateOrderingDto createOrderDto)
        {
            await _httpClient.PostAsJsonAsync("orderings", createOrderDto);
        }

        public async Task DeleteOrderAsync(int orderingId)
        {
            await _httpClient.DeleteAsync($"orderings/{orderingId}");
        }
    }
}
